...
public static class BootstrapProductionSetup
{
    ...
    [MenuItem("Tools/DATN/One-off Setup/Bootstraps/Bootstrap Production Setup")]
    ...
    [MenuItem("Tools/DATN/One-off Setup/Bootstraps/Sprint 4 Data Only")]
    ...
    [MenuItem("Tools/DATN/One-off Setup/Bootstraps/Sprint 2 Quality Data Only")]
    ...
    [MenuItem("Tools/DATN/One-off Setup/Bootstraps/Sprint 5 Narrative Data Only")]
    ...
    [MenuItem("Tools/DATN/One-off Setup/Bootstraps/Sprint 5 Research Data Only")]
    ...
    [MenuItem("Tools/DATN/One-off Setup/Bootstraps/Vertical Slice VS-1 Crop Data Only")]
    ...
    [MenuItem("Tools/DATN/One-off Setup/Bootstraps/Validate Production Setup")]
    ...
    private static void ConfigureM3ResourceData()
    {
        ...
        ConfigureResourceEntity(
            ...
            rewardExp: 12,
            growthDaysPerStage: new[] { 2, 3, 999 },
            useProceduralHitMotion: false);
        ...
        ConfigureResourceEntity(
            ...
            rewardExp: 14,
            growthDaysPerStage: new[] { 1, 2, 999 },
            useProceduralHitMotion: false);
    ...
    private static void ConfigureResourceEntity(
        ...
        DropEntry[] drops,
        int rewardExp,
        int[] growthDaysPerStage = null,
        bool useProceduralHitMotion = true)
    {
        ...
        resource.modules.Add(new ResourceHitReactionModule { useProceduralMotion = useProceduralHitMotion });
        if (growthDaysPerStage != null && growthDaysPerStage.Length > 0)
            resource.modules.Add(new ResourceGrowthModule { stages = CreateResourceGrowthStages(resource.icon, growthDaysPerStage) });
    ...
    private static GrowthStage[] CreateResourceGrowthStages(Sprite stageSprite, int[] growthDaysPerStage)
    {
        var source = growthDaysPerStage != null && growthDaysPerStage.Length > 0
            ? growthDaysPerStage
            : new[] { 1, 2, 999 };

        var stages = new GrowthStage[source.Length];
        for (int index = 0; index < source.Length; index++)
        {
            stages[index] = new GrowthStage
            {
                sprite = stageSprite,
                daysToGrow = Mathf.Max(1, source[index]),
                canHarvest = index == source.Length - 1
            };
        }
    ...
    private static void ConfigureOreData()
    {
        foreach (var ore in LoadAllEntities("Assets/Project/ScriptableObjects/WorldObjects/Resources", "OreNode_"))
        {
            ...
            EnsureModule<ResourceHitReactionModule>(ore).useProceduralMotion = false;
            EnsureModule<ResourceGrowthModule>(ore).stages = CreateResourceGrowthStages(ore.icon, new[] { 2, 3, 999 });
